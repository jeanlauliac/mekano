...
function outputRelations(graph, output, cb) {
    ;(function next(i) {
        if (i >= graph.rels.length) return cb()
        var rel = graph.rels[i]
        var relJsonName = JSON.stringify('__rel__' + rel.outFiles[0].path)
        var str = util.format(REL_TPL, relJsonName, JSON.stringify(rel.recipe))
        str += util.format(IN_EDGE_TPL, dotifyFiles(rel.inFiles), relJsonName)
        str += util.format(OUT_EDGE_TPL, relJsonName, dotifyFiles(rel.outFiles))
        output(str, function (err) {
            if (err) return cb(err)
            return next(i + 1)
        })
