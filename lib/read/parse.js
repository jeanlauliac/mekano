        ...
        if (self._accept(Token.EQUAL, 1))
            return self._parseBind(function (err, value) {
                ...
                if (unit.binds.hasOwnProperty(value))
                    return cb(new Error(util.format(DUP_BIND, value.name)))
        ...
        if (self._accept(Token.COLON, 1))
            return self._parseRecipe(function (err, recipe) {
                ...
                if (unit.recipes.hasOwnProperty(recipe))
                    return cb(new Error(util.format(DUP_REC, recipe.name)))
...
Parser.prototype._parseBind = function (cb) {
    ...
    this._read(4, function (err) {
        ...
        var bind = new ast.Bind(self._value(0), interpol)
        self._consume(4)
        return cb(null, bind)
    })
}
...
Parser.prototype._parseRecipe = function (cb) {
    ...
    this._read(4, function (err) {
        ...
        if (!self._accept(Token.SEMICOLON, 3))
            return cb(new Error(EXPECTED_SEMICOLON))
        var interpol = new Interpolation(self._value(2))
        var recipe = new ast.Recipe(self._value(0), interpol)
        self._consume(4)
