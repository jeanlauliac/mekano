...
Parser.prototype._write = function (chunk, encoding, cb) {
    ...
    if (this._pendingCb) return cb()
    // FIXME: after we got Token.END, the cb is hanging ad vitam eternam
    // Should transform the whole parse() as a Readable consumer rather than
    // a write stream
