...
var identifyFiles = require('../../lib/update/identify-files')
...
test('update.identifyFiles()', function (t) {
    var targets = TEST_GRAPH.getFilesByPaths(['a.out'])
    var files = identifyFiles(TEST_GRAPH, targets)
    t.equal(files[0].path, 'a.out')
    t.equal(files[1].path, 'foo.o')
    t.equal(files[2].path, 'foo.c')
    t.equal(files[3].path, 'bar.o')
    t.equal(files[4].path, 'bar.c')
    t.end()
})
...
test('update.identifyFiles() cycle', function (t) {
    var targets = TEST_GRAPH_CYCLE.getFilesByPaths(['a.out'])
    try {
        identifyFiles(TEST_GRAPH, targets)
    } catch (err) {
